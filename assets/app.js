...
(function () {
  ...
  function boot() {
    ...
    function head(p) {
      var eye = t(p.eyebrow)
        ? '<p class="page-head__eyebrow">' + esc(t(p.eyebrow)) + "</p>" : "";
      var sub = t(p.subtitle)
        ? '<p class="page-head__sub">' + esc(t(p.subtitle)) + "</p>" : "";
      return '<header class="page-head">' + eye + "<h1>" + esc(t(p.title)) + "</h1>" + sub + "</header>";
    }
    ...
    var RENDERERS = {
      ...
      bento: function (p) {
        var tiles = (p.tiles || []).map(function (tile) {
          return '<article class="tile tile--' + esc(tile.size || "sm") + (tile.accent ? " tile--accent" : "") + '" data-item>' +
            ...
            (t(tile.value) ? '<b class="tile__value">' + esc(t(tile.value)) + "</b>" : "") +
      ...
      /* ---- glossary: searchable definition list (custom) ---- */
      glossary: function (p) {
        var rows = (p.terms || []).map(function (term) {
          var hay = (t(term.term) + " " + t(term.def)).toLowerCase();
          return '<div class="gl-item" data-item data-q="' + esc(hay) + '">' +
            '<dt class="gl-term">' + esc(t(term.term)) + "</dt>" +
            '<dd class="gl-def">' + esc(t(term.def)) + "</dd></div>";
        }).join("");
        return head(p) +
          '<div class="toolbar"><input id="search" class="search" type="search" autocomplete="off" ' +
            'placeholder="' + (L.state.lang === "en" ? "Search terms…" : "搜尋術語…") + '" ' +
            'aria-label="' + (L.state.lang === "en" ? "Search terms" : "搜尋術語") + '" /></div>' +
          '<p class="result-count" id="glossaryCount" aria-live="polite"></p>' +
          '<dl class="glossary" id="glossary">' + rows + "</dl>";
      },

      /* ---- flashcards: tap-to-flip deck stepped one card at a time (custom) ---- */
      flashcards: function (p) {
        var hintFront = L.state.lang === "en" ? "Term" : "詞";
        var hintBack  = L.state.lang === "en" ? "Meaning" : "義";
        return head(p) +
          '<div class="fc-stage" data-item>' +
            '<div class="fc-card" id="fcCard" role="button" tabindex="0" ' +
              'aria-label="' + (L.state.lang === "en" ? "Flashcard, tap to flip" : "字卡,點擊翻面") + '">' +
              '<div class="fc-faces">' +
                '<div class="fc-face fc-front"><span class="fc-hint">' + esc(hintFront) + '</span>' +
                  '<p class="fc-text" id="fcFront"></p></div>' +
                '<div class="fc-face fc-back"><span class="fc-hint">' + esc(hintBack) + '</span>' +
                  '<p class="fc-text" id="fcBack"></p></div>' +
              "</div>" +
            "</div>" +
            '<div class="fc-controls">' +
              '<button class="btn-soft" id="fcPrev" type="button">' + esc(L.state.lang === "en" ? "Prev" : "上一張") + "</button>" +
              '<span class="fc-progress" id="fcProgress" aria-live="polite"></span>' +
              '<button class="btn-soft" id="fcNext" type="button">' + esc(L.state.lang === "en" ? "Next" : "下一張") + "</button>" +
            "</div>" +
            '<p class="fc-tip">' + esc(L.state.lang === "en" ? "Tap the card to flip · arrow keys to navigate." : "點卡片翻面 · 方向鍵切換。") + "</p>" +
          "</div>";
      },

      /* ---- quiz: multiple choice with instant feedback + score (custom) ---- */
      quiz: function (p) {
        var body = (p.questions || []).map(function (item, qi) {
          var opts = (item.options || []).map(function (op, oi) {
            return '<button class="quiz-opt" type="button" data-q="' + qi + '" data-o="' + oi + '">' +
              esc(t(op)) + "</button>";
          }).join("");
          return '<div class="quiz-q" data-item data-qi="' + qi + '" data-answer="' + esc(String(item.answer)) + '">' +
            '<p class="quiz-prompt"><span class="quiz-num">' + (qi + 1) + "</span>" + esc(t(item.q)) + "</p>" +
            '<div class="quiz-opts">' + opts + "</div>" +
            '<p class="quiz-explain" hidden>' + esc(t(item.explain)) + "</p>" +
          "</div>";
        }).join("");
        return head(p) +
          '<div class="quiz-score" id="quizScore" aria-live="polite"></div>' +
          '<div class="quiz" id="quiz">' + body + "</div>";
      }
    };
    ...
    var WIRE = {
      ...
      glossary: function () {
        var search = document.getElementById("search");
        var items = [].slice.call(pageEl.querySelectorAll(".gl-item"));
        ...
        function paint() {
          ...
          if (count) count.textContent = shown + (L.state.lang === "en" ? " term(s)" : " 個術語");
        }
      ...
      flashcards: function (p) {
        ...
        function show() {
          ...
          front.textContent = t(cards[i].front);   // textContent: safe, no escaping needed
          back.textContent = t(cards[i].back);
          if (prog) prog.textContent = (i + 1) + " / " + cards.length;
        }
        function flip() { card.classList.toggle("flipped"); }
        function go(d) { i = (i + d + cards.length) % cards.length; show(); }
      ...
      quiz: function (p) {
        var qs = p.questions || [], total = qs.length;
        var answered = {}, correct = 0;
        var scoreEl = document.getElementById("quizScore");
        function updateScore() {
          var done = Object.keys(answered).length;
          scoreEl.textContent = L.state.lang === "en"
            ? ("Score " + correct + " / " + total + (done === total ? " — done!" : " · " + done + " answered"))
            : ("得分 " + correct + " / " + total + (done === total ? " —— 完成!" : " · 已作答 " + done));
        }
        updateScore();
        [].forEach.call(pageEl.querySelectorAll(".quiz-q"), function (qEl) {
          var qi = qEl.dataset.qi;
          var answer = parseInt(qEl.dataset.answer, 10);
          var explain = qEl.querySelector(".quiz-explain");
          [].forEach.call(qEl.querySelectorAll(".quiz-opt"), function (btn) {
            btn.addEventListener("click", function () {
              if (answered[qi]) return;                 // lock each question after first answer
              answered[qi] = true;
              var chosen = parseInt(btn.dataset.o, 10);
              qEl.classList.add("quiz-q--answered");
              [].forEach.call(qEl.querySelectorAll(".quiz-opt"), function (b, oi) {
                b.disabled = true;
                if (oi === answer) b.classList.add("quiz-opt--correct");
                else if (oi === chosen) b.classList.add("quiz-opt--wrong");
              });
